			...
			<div class="col-3">설문 부가 설명</div>
				...
				<div class="col-3">
					<c:if test="${SLD.anonymityCheckCode eq '20001'}">
						<input type="radio" name="anonymityCheckCode" value="20001"
							checked>익명
...
<!-- 문항 관리 진택-->
<div class="Item_box" style="float: right;">
			...
			<div class="question_content_area" id="question_add">
				<div class="input-group" id="question_content">
					...
					<textarea class="form-control" aria-label="문제 입력칸"
						name="questionContent" value="${sqd.questionContent}"></textarea>
				</div>
				<!-- 문제 추가 버튼  -->
				<button type="submit" class="btn btn-outline-primary" id="add_btn">문제
					추가</button>
			</div>
		...
		$.ajax({
			...
			success: function (data) {
				...
				$.ajax({ 
					...
					cache: false, 
					timeout: 600000, 
					beforeSend : function() { // 전송 전 실행 코드 
					},
				
					success: function (data) { // 전송 후 성공 시 실행 코드 console.log(data);
